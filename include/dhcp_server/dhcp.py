...
class DHCPServerConfiguration(object):
    ...
    dhcp_offer_after_seconds = 2
    dhcp_acknowledge_after_seconds = 2
...
class DHCPServer(object):
    ...
    def get_ip_address(self, packet):
        ...
        if not any([host.ip == ip for host in known_hosts]):
            logging.debug("add %s %s %s", mac_address, ip, packet.host_name)
    ...
    def broadcast(self, packet):
        ...
        for addr in self.server_identifiers:
            ...
            broadcast_socket.bind((addr, local_port))
            try:
                data = packet.to_bytes()
                broadcast_socket.sendto(data, ('255.255.255.255', remote_port))
                broadcast_socket.sendto(data, (addr, remote_port))
            finally:
